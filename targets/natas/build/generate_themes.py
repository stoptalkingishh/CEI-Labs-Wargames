 ...
 *
 * Base typography/readability rules below are shared across both
 * states, deliberately unconditional -- the level content itself
 * (index.php, etc.) is plain unstyled HTML with no font-size/color/
 * padding of its own, so without this it renders at the browser's tiny
 * unstyled default. Explicit color is required, not optional: leaving
 * it unset against our own dark background-color would rely on the
 * browser's own default text color (usually black) or an auto-dark-mode
 * heuristic to still produce readable contrast, which is exactly the
 * kind of thing that's fine in one browser/version and broken in
 * another -- confirmed live as illegibly small before this fix, on the
 * venue's actual Chromium build.
 */
body {
  color: #e8eef2;
