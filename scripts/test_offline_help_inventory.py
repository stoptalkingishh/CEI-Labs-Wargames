...
class InventoryTests(unittest.TestCase):
 ...
 def test_krypton_tier_one_matches_inventory(self):
  ...
  self.assertIn("base64 --help",hints["krypton-00"][0][0]); self.assertIn("xxd --help",hints["krypton-06"][0][0])
  for n in range(1,6):
   text=hints["krypton-%02d"%n][0][0]; self.assertNotIn("--help",text); self.assertNotIn(" -h",text); self.assertNotIn("man ",text)
 ...
 def test_natas_tier_one_matches_inventory(self):
  ...
  for n in command_ids:
   text=hints["natas-%02d"%n][0][0]; self.assertIn("curl --help",text); self.assertIn("attacker workstation",text)
  for n in set(range(15))-command_ids:
   text=hints["natas-%02d"%n][0][0]; self.assertNotIn("curl --help",text); self.assertNotIn("man ",text)
