...
challenges_data = [
    ...
    {"id": "sentinel-24", "name": "Sentinel 24: Endpoint Enrollment Evidence", "points": 500,
     "goal": "Corroborate endpoint enrollment by matching a simulated local enrollment record across inventory, transcript, and key lifecycle evidence.",
     "task": "Work as `sentinel24`. Review only the static local file `~/evidence/endpoint-enrollment.txt`; do not contact an endpoint, agent, or manager. Submit the documented endpoint ID, enrollment record ID, enrollment status, and key status tuple through `sentinel-submit`. " + DEFERRED_EXPANSION_STATUS, "flag": _dynamic("sentinel-24")},
...
HINTS = {
    ...
    "sentinel-24": ["Enrollment is supported only when the inventory, transcript, and key lifecycle sections share the same enrollment record ID.", "This simulated record is static evidence only: do not contact an endpoint, agent, or manager.", "Read `endpoint-enrollment.txt`, then submit the endpoint_id, enrollment_record_id, enrollment_status, and key_status tuple through `sentinel-submit`."],
