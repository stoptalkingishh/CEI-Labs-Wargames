...
challenges_data = [
    ...
    {"id": "sentinel-26", "name": "Sentinel 26: Network Inventory Review", "points": 550,
     ...
     "task": "Work as `sentinel26`. Review only the static local file `~/evidence/network-inventory.txt`; do not scan networks or contact external services. Determine which observed device lacks the required DHCP registration, then submit the documented structured inventory-review tuple through `sentinel-submit`. " + DEFERRED_EXPANSION_STATUS, "flag": _dynamic("sentinel-26")},
...
HINTS = {
    ...
    "sentinel-26": ["Compare each ARP-observed MAC with the DHCP registration state and the engineering-zone policy.", "The inventory is synthetic evidence only: do not scan a network, probe a device, or contact an external service.", "Read `network-inventory.txt`, determine the policy outcome from the recorded evidence, then submit the device_mac, zone, and disposition tuple through `sentinel-submit`."],
