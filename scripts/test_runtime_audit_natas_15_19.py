...
spec = importlib.util.spec_from_file_location("audit", pathlib.Path(__file__).with_name("runtime_audit_natas_15_19.py"))
audit = importlib.util.module_from_spec(spec)
spec.loader.exec_module(audit)


class BatchAAuditTests(unittest.TestCase):
    def test_local_target_and_adjacent_secrets_are_required(self):
        secrets = {"natas%d" % n: "test-%d" % n for n in range(15, 21)}
