# Wargame Storyboard: Captured Art

Per-level art drawn against the prompts in [`wargame-story.md`](wargame-story.md) -- one piece per level, following that level's story beat and title.

**Status: proposal, not shipped.** Nothing in this file is wired into any Dockerfile or banner generator. The banners players currently see are still the programmatic storyboard (chapter boxes, transmission strips, depth shafts) described in [`wargame-themes.md`](wargame-themes.md). This file exists so the story-driven art can be reviewed as a complete set before deciding whether to adopt it, and which pieces need rework.

All pieces obey the shared constraints in `wargame-story.md`: single-width characters only, <= 80 columns per line, and no written text -- the art depicts, it never labels. Rendering depends on the viewer's font; these use box-drawing and block-element glyphs, so a font with poor coverage will show gaps.

Each track carries a recurring anchor so a piece reads as belonging to its track at a glance: Bandit the starlit sky and ground line, Krypton the listening dish, Natas a depth gauge down the left edge (filled above, marker at this floor, empty below).

Bandit's outlaw and Krypton's operator appear in every one of their pieces, sharing a baseline with whatever they are acting on. Natas deliberately has no figure: its researcher is the person at the screen, outside the frame looking in, so the pieces show what is on the screen rather than a bystander standing next to it.

## Bandit -- "The Vault at Dryrock"

### bandit0: The First Step

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▜          ▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌          ▐          ▌                ▐
▌          ▐          ▌                ▐
▌          ▐     ○    ▌                ▐
▌          ▐    /|\   ▌                ▐
▙▄▄▄▄▄▄▄▄▄▄▟    / \   ▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit1: Dashed Hopes

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌  ○      ·    ████ ▐
▌ /|\     ·    ████ ▐
▌ / >     ·    ████ ▐
▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit2: Spaces in Places

```
  ✦        ·          ✧         ·        ✦        ·
█████████████  ○   █████████████
█████████████  |\  █████████████
█████████████  /\  █████████████
█████████████      █████████████
█████████████      █████████████
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit3: Hidden in Plain Sight

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌ ┌──┐  ┌──┐  ◈◈◈  ┌──┐  ┌──┐    ○  ▐
▌ │▤▤│  │▤▤│  ◈◈◈  │▤▤│  │▤▤│   /|\ ▐
▌ └──┘  └──┘  ◈◈◈  └──┘  └──┘   / \ ▐
▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit4: Human Readable

```
  ✦        ·          ✧         ·        ✦        ·
      ▕▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▏
      ▕ ───────────── ▏   ╲ │ ╱
      ▕ ───────────── ▏   ─ ◉ ─
 ○    ▕ ───────────── ▏   ╱ │ ╲
/|\   ▕ ───────────── ▏     █
/ \   ▕▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▏     █
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit5: The Needle

```
  ✦        ·          ✧         ·        ✦        ·
\○/   ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤
 |    ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ◈◈◈ ▤▤▤ ▤▤▤ ▤▤▤
/ \   ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤ ▤▤▤
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit6: Server Search

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▜  ▛▀▀▀▀▀▀▜  ▛▀▀▀▀▀▀▜  ▛▀▀▀▀▀▀▜
 ○    ▌ ████ ▐  ▌ ████ ▐  ▌ ████ ▐  ▌ ████ ▐
/|\   ▌ █▯██ ▐  ▌ █▯██ ▐  ▌ █▯██ ▐  ▌ █▯██ ▐
/ >   ▙▄▄▄▄▄▄▟  ▙▄▄▄▄▄▄▟  ▙▄▄▄▄▄▄▟  ▙▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit7: The Millionth Word

```
  ✦        ·          ✧         ·        ✦        ·
 ○    ┌──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┐
/|\   │▤▤│▤▤│▤▤│▤▤│▤▤│▤▤│◈◈│▤▤│▤▤│▤▤│▤▤│▤▤│
/ \   └──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┴──┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit8: The Only One

```
  ✦        ·          ✧         ·        ✦        ·
 ○    ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢
/|\   ▢  ▢  ▢  ▢  ◈  ▢  ▢  ▢  ▢  ▢  ▢
/ \   ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢  ▢
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit9: Strings Attached

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
\○/   ▌ ╱╲ ╱╲╱ ╲╱╲ ╱ ╲╱╲ ╱ ▐
 |    ▌  ╲╱ ╲ ╱╲ ╱╲╱ ╲ ╱╲╱ ▐
/ \   ▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit10: Base Operations

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▀▀▀▜       ▛▀▀▀▀▀▀▀▜
      ▌ ▓▓▓▓▓▓▓ ▐       ▌ ▤ ▤ ▤ ▐
 ○    ▌ ▓▓▓▓▓▓▓ ▐  ══▶  ▌ ▤ ▤ ▤ ▐
/|\   ▌ ▓▓▓▓▓▓▓ ▐       ▌ ▤ ▤ ▤ ▐
 |\   ▙▄▄▄▄▄▄▄▄▄▟       ▙▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit11: Substitution

```
  ✦        ·          ✧         ·        ✦        ·
        ▛▀▀▀▀▜            ▛▀▀▀▀▜
        ▌    ▐            ▌    ▐
 ○  ◈   ▌ ◈  ▐            ▌ ▢  ▐
/|\     ▌    ▐   ◀────▶   ▌    ▐
/ \     ▙▄▄▄▄▟            ▙▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit12: Matryoshka

```
  ✦        ·          ✧         ·        ✦        ·
      ┌───────────────────────┐
      │  ┌─────────────────┐  │
      │  │  ┌───────────┐  │  │
      │  │  │  ┌─────┐  │  │  │
      │  │  │  │  ◈  │  │  │  │
      │  │  │  └─────┘  │  │  │
\○/   │  │  └───────────┘  │  │
 |    │  └─────────────────┘  │
/ \   └───────────────────────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit13: Private Keys

```
  ✦        ·          ✧         ·        ✦        ·
                            ▛▀▀▀▀▀▜
          ▄▄▄               ▌     ▐
 ○       █   █▄▄▄▄▄▄▄  ══▶  ▌  ◉  ▐
/|\      █   █▀▀▘▝▀▀▘       ▌     ▐
/ \       ▀▀▀               ▙▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit14: Port Submission

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
      ▌ █████████████     ███████████████ ▐
 ○    ▌ █████████████ ▭▶  ███████████████ ▐
/|\   ▌ █████████████     ███████████████ ▐
/ >   ▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit15: SSL Encryption

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
      ▌ ████████████ ▚▚▚▚▚▚ ████████████ ▐
 ○    ▌ ████████████ ▚ ▭▶ ▚ ████████████ ▐
/|\   ▌ ████████████ ▚▚▚▚▚▚ ████████████ ▐
/ >   ▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit16: SSL Port Scan

```
  ✦        ·          ✧         ·        ✦        ·
      ┌──┐  ┌──┐  ┌──┐  ┌──┐  ┌──┐  ┌──┐
 ○    │▚▚│  │▚▚│  │▚▚│  │▚▚│  │▚▚│  │▚▚│
/|\   │▚▚│  │▚▚│  │▚▚│  │▚▚│  │▚▚│  │▚▚│
/ >   └──┘  └──┘  └──┘  └──┘  └──┘  └──┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit17: File Comparisons

```
  ✦        ·          ✧         ·        ✦        ·
      ┌─────────────┐          ┌─────────────┐
      │ ─────────── │          │ ─────────── │
      │ ─────────── │          │ ─────────── │
 ○    │ ───◈─────── │    ╭─╮   │ ─────────── │
/|\   │ ─────────── │    ╰─╯   │ ─────────── │
/ \   └─────────────┘          └─────────────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit18: Shell Bypass

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌ ███████████ ▐▌ ██████     ▕▔▔▔▔▔▔▔▔▏ ▐
▌ ███████████ ▐▌ ██████  ○  ▕        ▏ ▐
▌ ███████████ ▐▌ ██████ /|\ ▕        ▏ ▐
▌ ███████████ ▐▌ ██████ / \ ▕▁▁▁▁▁▁▁▁▏ ▐
▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit19: SUID Escalation

```
  ✦        ·          ✧         ·        ✦        ·
      ▕▔▔▔▔▔▔▔▔▔▏
      ▕ ███████ ▏
 ○    ▕ ███████ ▏   ══▶    ○
/|\   ▕ ███████ ▏         /█\
/ \   ▕▁▁▁▁▁▁▁▁▁▏         / \
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit20: Port Listener Connection

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌                 ┌─────┐ ▐
▌                 │     │ ▐
▌  ○      ) ) )   │  ◐  │ ▐
▌ /|\   ▶ ▶ ▶ ▶   │     │ ▐
▌ / \             └─────┘ ▐
▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit21: Cron Jobs

```
  ✦        ·          ✧         ·        ✦        ·
        ▄▄▄▄▄
       ███████
      █████████
       ▀▀▀◍▀▀▀
      ▛▀▀▀▀▀▀▀▀▜
      ▌   12   ▐
      ▌   ◷    ▐    ▄ ▄ ▄ ▄
      ▌   6    ▐   ▄████████▄
\○/   ▙▄▄▄▄▄▄▄▄▟   █   ◉    █
 |    █████████    ▀████████▀
/ \   █████████     ▀ ▀ ▀ ▀
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit22: Cron Debugging

```
  ✦        ·          ✧         ·        ✦        ·
        ▄▄▄▄▄
       ███████
      █████████
       ▀▀▀◍▀▀▀
      ▛▀▀▀▀▀▀▀▀▜
      ▌   12   ▐
      ▌   ◷    ▐    ▄ ▄ ▄ ▄
      ▌   6    ▐   ▄████████▄
\○/   ▙▄▄▄▄▄▄▄▄▟   █   ◉    █
 |    █████████    ▀████████▀
/ \   █████████     ▀ ▀   ▀
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit23: Cron Scripting

```
  ✦        ·          ✧         ·        ✦        ·
        ▄▄▄▄▄
       ███████      ▄ ▄ ▄ ▄
      █████████    ▄████████▄
       ▀▀▀◍▀▀▀     █   ◉    █
      ▛▀▀▀▀▀▀▀▀▜   ▀████████▀
      ▌   12   ▐    ▀ ▀ ▀ ▀
      ▌   ◷    ▐
      ▌   6    ▐   ▕▔▔▔▔▔▔▔▏
\○/   ▙▄▄▄▄▄▄▄▄▟   ▕ ───── ▏
 |    █████████    ▕ ───── ▏
/ \   █████████    ▕▁▁▁▁▁▁▁▏
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit24: PIN Brute Force

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▀▀▀▜
      ▌ ▨ ▨ ▨ ▨ ▐
 ○    ▌ ? ? ? ? ▐
/|\   ▙▄▄▄▄▄▄▄▄▄▟
/ \     ███████
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit25: Shell Breakout

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌                                          ▐
▌ ▐▌▐▌▐▌▐▌▐▌▐▌▐▌▐▌  ○  ▐▌▐▌▐▌▐▌▐▌▐▌▐▌▐▌  ▚ ▐
▌ ▐▌▐▌▐▌▐▌▐▌▐▌▐▌▐▌ /|\ ▐▌▐▌▐▌▐▌▐▌▐▌▐▌▐▌    ▐
▌ ▐▌▐▌▐▌▐▌▐▌▐▌▐▌▐▌ / \ ▐▌▐▌▐▌▐▌▐▌▐▌▐▌▐▌    ▐
▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit26: Text UI Breakout

```
  ✦        ·          ✧         ·        ✦        ·
▛▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▜
▌                          ▐
▌ ▐▌▐▌▐▌▐▌  ○  ▐▌▐▌▐▌▐▌  ▚ ▐
▌ ▐▌▐▌▐▌▐▌ /|\ ▐▌▐▌▐▌▐▌    ▐
▌ ▐▌▐▌▐▌▐▌ / \ ▐▌▐▌▐▌▐▌    ▐
▙▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▟
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit27: Git Clone

```
  ✦        ·          ✧         ·        ✦        ·
       ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐
 ○ ▤   │ ▤▤▤▤ │  │ ▤▤▤▤ │  │ ▤▤▤▤ │  │ ▤▤▤▤ │
/|\    │ ▤▤▤▤ │  │ ▤▤▤▤ │  │ ▤▤▤▤ │  │ ▤▤▤▤ │
/ \    └──────┘  └──────┘  └──────┘  └──────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit28: Git Commits

```
  ✦        ·          ✧         ·        ✦        ·
      ┌───────────┬───────────┐
      │ ●──────── │ ●──────── │
      │ ●──────── │ ●──────── │
 ○    │ ●──────── │ ●──────── │
/|\   │ ●──────── │ ●──────── │
/ \   └───────────┴───────────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit29: Git Branches

```
  ✦        ·          ✧         ·        ✦        ·
                          ┌──────────┐
                    ╱─────│ ▤▤▤▤▤▤▤▤ │
      ┌──────────┐ ╱      └──────────┘
      │ ▤▤▤▤▤▤▤▤ │
 ○    └──────────┘ ╲      ┌──────────┐
/|\                 ╲─────│ ▤▤▤▤▤▤▤▤ │
/ \                       └──────────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit30: Git Tags

```
  ✦        ·          ✧         ·        ✦        ·
      ┌───────────┬───────────┐
      │ ●──────── │ ●──────── │ ◀▮
      │ ●──────── │ ●──────── │
 ○    │ ●──────── │ ●──────── │ ◀▮
/|\   │ ●──────── │ ●──────── │
/ \   └───────────┴───────────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit31: Git Push

```
  ✦        ·          ✧         ·        ✦        ·
        ┌───────────┬───────────┐
        │ ●──────── │ ●──────── │
 ○  ✎   │ ●──────── │ ●──────── │
/|\     │ ●──────── │ ●─── ✎    │
/ \     └───────────┴───────────┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit32: Shell Overrides

```
  ✦        ·          ✧         ·        ✦        ·
      ▛▀▀▀▀▀▀▀▀▀▀▀▀▀▜
      ▌    ┌───┐    ▐
      ▌    │ ◉ │    ▐
      ▌    └───┘    ▐
      ▌      ✗      ▐
      ▙▄▄▄▄▄▄▄▄▄▄▄▄▄▟
             ▲
 ○         ┌───┐
/|\        │ ◉ │
/ \        └───┘
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

### bandit33: Final Escape

```
  ✦        ·          ✧         ·        ✦        ·
█████████████            ·      ✦
██████▛▀▀▀▀▀▀               ·
██████▌          ○ ▤    ✧     ·
██████▙▄▄▄▄▄▄   /|\       ·   ✦
█████████████   / \
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
```

## Krypton -- "Signal from the Dark"

### krypton0: Base64 Decoding

```
 ·     ★        ·        ☆       ·      ★       ·
      ╲   ▁▁▁▁▁   ╱
       ╲ ▕     ▏ ╱    ≈≈≈    ▨▧▨▧▩▨
 ○      ▕▔▔▔▔▔▔▔▏     ≈≈≈≈   ▧▩▨▧▨▩
/|\      ▔▔▔█▔▔▔      ≈≈≈    ▨▧▩▨▧▨
/ \         █
```

### krypton1: ROT13 Substitution Cipher

```
 ·     ★        ·        ☆       ·      ★       ·
                    ╲  │  ╱   ↻
                     ╲ │ ╱
 ○    ╲▁▁▁╱      ───── ◉ ─────
/|\    ▕ ▏           ╱ │ ╲
/ \     █           ╱  │  ╲
```

### krypton2: Caesar Cipher (Unknown Shift)

```
 ·     ★        ·        ☆       ·      ★       ·
                    ╲  ┊  ╱   ↻ ?
                     ╲ ┊ ╱
 ○    ╲▁▁▁╱      ┄┄┄┄┄ ◉ ┄┄┄┄┄
/|\    ▕ ▏           ╱ ┊ ╲
/ \     █           ╱  ┊  ╲
```

### krypton3: Frequency Analysis

```
 ·     ★        ·        ☆       ·      ★       ·
              │
              │              ▉
              │        ▉     ▉  ▉
 ○    ╲▁▁▁╱   │     ▉  ▉  ▉  ▉  ▉     ▉
/|\    ▕ ▏    │  ▉  ▉  ▉  ▉  ▉  ▉  ▉  ▉  ▉
/ \     █     └──────────────────────────────────
```

### krypton4: Vigenere Cipher (Known Key Length)

```
 ·     ★        ·        ☆       ·      ★       ·
               ▟▀▀▙   ▟▀▀▙   ▟▀▀▙   ▟▀▀▙   ▟▀▀▙
              ╱    ╲ ╱    ╲ ╱    ╲ ╱    ╲ ╱    ╲
 ○    ╲▁▁▁╱         V      V      V      V
/|\    ▕ ▏    ├──────┼──────┼──────┼──────┼─────┤
/ \     █     ◀─────▶◀─────▶◀─────▶◀─────▶◀────▶
```

### krypton5: Vigenere Cipher (Kasiski Test)

```
 ·     ★        ·        ☆       ·      ★       ·
 ○    ╲▁▁▁╱   ░░░ ███ ░░░░ ░░░ ███ ░░░ ░░░ ███ ░░░
/|\    ▕ ▏        └─────────────┘     └─────────┘
/ \     █         ◀─────────────▶     ◀─────────▶
```

### krypton6: Stream Cipher / LFSR

```
 ·     ★        ·        ☆       ·      ★       ·
                  ┌─────────────┐
                  │             │
                  ▼    ┌─┬─┬─┬─┬─┬─┬─┐
 ○    ╲▁▁▁╱       ▲    │░│█│░│█│█│░│█│
/|\    ▕ ▏        │    └─┴─┴─┴─┴─┴─┴─┘
/ \     █         └───────────────────┘
```

## Natas -- "Into the Mirror"

### natas0: View Source

```
█   ┌────────────────────────────┐
░   │ ● ● ●                      │
░   ├────────────────────────────┤
░   │ ████████████████████       │
░   │ ████████████████████       │
░   └────────────────────────────┘
░         ≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈
░   ┌────────────────────────────┐
░   │ ● ● ●                      │
░   ├────────────────────────────┤
░   │ < >< >< >< >< >< ><        │
░   │ < >< >< >< >< >< ><        │
░   └────────────────────────────┘
```

### natas1: Right-Click Block

```
▓   ┌────────────────────────────┐
█   │ ● ● ●                      │
░   ├────────────────────────────┤
░   │    ┌───────┐               │
░   │    │ ───── │               │
░   │    │ ───── │               │
░   │    │ ───── │  ✗            │
░   │    └───────┘               │
░   └────────────────────────────┘
░           ╲                        ╱
░            ╲──────────────────────╱
```

### natas2: Directory Traversal (Files)

```
█    ┌───┐       ┌───┐       ┌───┐       ┌ ─ ┐
░   ┌┘   └───┐  ┌┘   └───┐  ┌┘   └───┐  ┌┘   └ ─ ┐
░   │        │  │        │  │        │  ╎   ▨    ╎
░   └────────┘  └────────┘  └────────┘  └ ─  ─  ─┘
```

### natas3: Web Crawlers (Robots.txt)

```
▓           ┌───────────┐
█           │ ───────── │
░           │ ───────── │
░           └───────────┘
░               │
░               │
░     ══════════╪══════════════▶
```

### natas4: Referer Spoofing

```
▓   ┌────────┐        ┌────────┐
█   │╲      ╱│        │╲      ╱│
░   │ ╲    ╱ │  ═══▶  │ ╲ ✗  ╱ │
░   │  ╲__╱  │        │  ╲__╱  │
░   └────────┘        └────────┘
```

### natas5: Cookie Manipulation

```
▓   ┌─────────┐
▓   │ ▄▄▄▄▄▄▄ │
█   ├─────────┤
░   │ ◍  ◍  ◍ │
░   │ ◍  ◉  ◍ │
░   │ ◍  ◍  ◍ │
░   └─────────┘
```

### natas6: Hidden Inclusion Files

```
▓   ▨▨▨▨▨▨▨▨▨▨▨▨▨▨▨▨▨▨▨▨
▓   ▨▨▨▨▨▨▨▨▨▨▨▨▨╲▨▨▨▨▨▨
▓   ▨▨▨▨▨▨▨▨▨▨▨▨▨ ╲
█               ┌──────┐
░               │      │
░               └───┬──┘
░                   ▼
```

### natas7: Local File Inclusion (LFI)

```
▓   ┌────────────────────────┐
▓   │  ┌────────┐            │
█   │  │  ▤▤▤   │            │
░   │  └────┬───┘            │
░   └───────┼────────────────┘
░           └─ ─ ─ ─ ─ ─ ─ ─ ─ ─▶
```

### natas8: Reversing Crypto Schemes

```
▓     ▄▄▄
▓    █   █
▓   ┌─────┐
▓   │  ◉  │
▓   └─────┘
█   ≈≈≈≈≈≈≈≈≈≈≈≈≈≈≈
░     ▄▄▄
░    █   █▄▄▄▄▄▄
░    █   █▀▀▘▝▀▘
░     ▀▀▀
```

### natas9: Command Injection I

```
▓                          ▨▨▨▨
▓                           │
▓                           ▼
▓   ┌──────┬──────┬──────┬ ─ ─ ─┐
█   │ ▤▤▤▤ │ ▤▤▤▤ │ ▤▤▤▤ │      │
░   │ ▤▤▤▤ │ ▤▤▤▤ │ ▤▤▤▤ │      │
░   └──────┴──────┴──────┴ ─ ─ ─┘
```

### natas10: Command Injection II (Sanitization Bypass)

```
▓                   ▨▨▨▨
▓                    │
▓                ▓▓▓▓▓▓▓▓▓
▓                    ▼
▓   ┌──────┬──────┬ ─ ─ ─┐
█   │ ▤▤▤▤ │ ▤▤▤▤ │      │
░   │ ▤▤▤▤ │ ▤▤▤▤ │      │
░   └──────┴──────┴ ─ ─ ─┘
```

### natas11: XOR Encryption Bypass

```
▓          ╲         ╱
▓           ╲       ╱
▓            ╲     ╱
▓             ╲   ╱
▓              ╲ ╱
▓               ◉
▓              ╱ ╲
▓             ╱   ╲
█            ╱     ╲
░           ╱       ╲
░          ╱         ╲
```

### natas12: Arbitrary File Upload (Web Shell)

```
▓   ██████████    ██████████
▓   ██████████    ██████████
▓   ██████████    ██████████
▓              ▲
▓           ┌──────┐
▓           │ ▨▨▨▨ │
█           │ ▨▨▨▨ │
░           └──────┘
```

### natas13: File Upload Bypass (Magic Bytes)

```
▓          ┌──────┐
▓          │══════│  ✗
▓          │ ▨▨▨▨ │
▓          │══════│
█          └──────┘
```

### natas14: SQL Injection (SQLi)

```
▓   ┌──┬──┬──┬──┬──┬──┐
▓   │▤▤│▤▤│▤▤│▤▤│▤▤│▤▤│
▓   ├──┼──┼──┼──┼──┼──┤
▓   │▤▤│▤▤│▤▤│▤▤│▤▤│▤▤│
▓   └──┴──┴╲─┴──┴──┴──┘
▓           ╲
█            ╲ ▤▤  ▤▤  ▤▤
```
