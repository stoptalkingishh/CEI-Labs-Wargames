...
`hidden` to `visible`). All flags in `scripts/build_osint.py` have been
...
string in `build_osint.py`'s `FLAGS` dict to the new verified value, re-run:
...
> **Also re-check** `docs/tracks/osint/writeups.md` and `docs/tracks/osint/cheatsheet.md` —
...
- [ ] Confirm `docs/tracks/osint/writeups.md` sections and `FLAGS` stay 1:1
