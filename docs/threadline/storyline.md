# Threadline - Storyline & Faction Design
...
> **Why "Threadline":** a *threadline* is a military line of advance/operations - the path a force takes toward an objective. On a loom, the *warp threads* run lengthwise and are woven into the fabric: the hidden coordination node literally named **the Loom**. Follow the threads; walk the threadline; unmask the Loom.
...
reliability-scored, sourced statement, exactly the capstone `threadline-12` already
shapes.
...
- New-plot (non-HACKTORIA) threads - mostly the built **Gilded Generals**,
  **Chancellery**, **Wire Syndicate**, **Ashfall** factions - are written from
  scratch in `scripts/build_threadline.py` using each challenge's real premise from
