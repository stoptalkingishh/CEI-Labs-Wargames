...
### Restructured
  - **Documentation reorganized** into a proper structure with a root index
    (`docs/README.md`):
    - `docs/guides/` — evergreen operator/instructor/participant docs
      (runbook, cheatsheet, inventory, network access, self-hosting, staging,
      troubleshooting, security, etc.)
    - `docs/tracks/<game>/` — per-wargame docs, each with `README.md`,
      `learning-objectives.md`, `cheatsheet.md`, `writeups.md` (+ track extras)
    - `docs/reference/` and `docs/presentation/` kept as-is
    - `docs/archive/` — history: `events/`, `worklogs/`, `plans/`,
      `future-themes/`, `gaps/`, `art/`
  - `WORK_LOGS/` moved to `docs/archive/worklogs/`.
  - Every in-repo link/comment updated to the new paths; `README.md` now
    points at the docs index.
  - Generated `challenges/natas-15..34/challenge.yml` (stale committed
    generator output) untracked; they are git-ignored and regenerated by
    `build_natas.py`.
  - Follow-up documentation audit corrected stale inventory totals and links,
    fixed the accidental `sentinel-ooda/sentinel-ooda/` nesting, and isolated
    Sentinel's hidden output under git-ignored `sentinel/` so it cannot pollute
    the staged 85-challenge validation set.
